<template>
  <div class="links" @click="closeContextMenu">
    <div class="line">
      ...
      <div class="action-buttons">
        ...
        <el-button type="primary" size="small" @click="triggerFileInput">
          ...
          <input type="file" ref="fileInputRef" accept=".html,.htm" style="display: none" @change="handleFileUpload" />
        </el-button>
      </div>
    </div>

    <!-- Swiper分页网格 -->
    <Swiper v-if="pagedShortcuts.length" :modules="[Pagination, Mousewheel]" :slides-per-view="1" :space-between="40"
      ...
      <SwiperSlide v-for="(page, pageIndex) in pagedShortcuts" :key="pageIndex">
        <el-row class="link-all" :gutter="20">
          <el-col v-for="item in page" :key="item.id" :xs="12" :sm="8" :md="6" :lg="4" class="shortcut-item-wrapper"
            @contextmenu.prevent="openContextMenu($event, item)" @click="jumpLink(item.url)">
    ...
    <!-- 自定义右键菜单 -->
    <Teleport to="body">
      <div v-if="contextMenuVisible" ref="contextMenuRef" class="custom-context-menu" :style="{
    ...
    <!-- 添加/编辑弹窗 -->
    <el-dialog v-model="dialogVisible" :title="dialogType === 'edit' ? '编辑捷径' : '添加捷径'" width="500px" align-center
      destroy-on-close>
...
const filteredData = computed(() => {
  ...
  return shortcutData.value.filter(item =>
...
const submitForm = () => {
  formRef.value?.validate((valid) => {
    ...
    if (dialogType.value === 'add') {
      const duplicate = shortcutData.value.some(item => item.name === formData.name || item.url === formData.url)
      if (duplicate) {
      ...
      const duplicate = shortcutData.value.some((item, idx) => idx !== index && (item.name === formData.name || item.url === formData.url))
      if (duplicate) {
        ElMessage.error('名称或链接已存在')
        return
      }
...
const confirmDelete = (item) => {
  ElMessageBox.confirm(`确认删除 “<el-text style="color:#E6A23C">${item.name}</el-text>” 捷径？此操作无法恢复！`, '删除捷径', {
